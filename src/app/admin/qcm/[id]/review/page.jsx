...
export default function ReviewPage({ params }) {
  ...
  return (
    <div style={{maxWidth:900, margin:'40px auto', padding:24}}>
      <h1>Review QCM</h1>
      <p><b>Language:</b> {qcm?.language}</p>
      <p><b>Status:</b> {qcm?.status}</p>

      <h2>Questions</h2>
      {questions.map((q, idx) => (
        <div key={q.id} style={{padding:16, border:'1px solid #eee', borderRadius:8, marginBottom:12}}>
          <div><b>Q{idx+1} ({q.skill_tag})</b></div>
      ...
      {shareUrl && (
        <div style={{marginTop:16}}>
          <b>Share URL:</b>
          <div><a href={shareUrl} target="_blank" rel="noreferrer">{shareUrl}</a></div>
        </div>
      )}
    </div>
  );
